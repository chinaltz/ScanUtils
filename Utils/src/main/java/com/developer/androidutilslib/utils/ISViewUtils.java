...
import com.developer.androidutilslib.view.refresh.ISListViewFooter;
import com.developer.androidutilslib.view.refresh.ISListViewHeader;
...
public class ISViewUtils {
    ...
    public static float dip2px(Context context, float dipValue) {
        DisplayMetrics mDisplayMetrics = ISAppUtils.getDisplayMetrics(context);
    ...
    public static int scaleValue(Context context, float value) {
        ...
        if (mDisplayMetrics.scaledDensity == ISAppConfig.UI_DENSITY) {
            //密度
            if (width > ISAppConfig.UI_WIDTH) {
                value = value * (1.3f - 1.0f / mDisplayMetrics.scaledDensity);
            } else if (width < ISAppConfig.UI_WIDTH) {
                value = value * (1.0f - 1.0f / mDisplayMetrics.scaledDensity);
            }
        } else {
            //密度小屏幕大:缩小比例
            float offset = ISAppConfig.UI_DENSITY - mDisplayMetrics.scaledDensity;
            if (offset > 0.5f) {
                value = value * 0.9f;
            } else {
                value = value * 0.95f;
    ...
    public static int scale(int displayWidth, int displayHeight, float pxValue) {
        ...
        try {
            ...
            float scaleWidth = (float) width / ISAppConfig.UI_WIDTH;
            float scaleHeight = (float) height / ISAppConfig.UI_HEIGHT;
    ...
    public static void scaleContentView(ViewGroup contentView) {
        ISViewUtils.scaleView(contentView);
    ...
    public static boolean isNeedScale(View view) {
        if (view instanceof ISListViewHeader) {
            return false;
        }

        if (view instanceof ISListViewFooter) {
            return false;
        }
        return true;
    }
    ...
    public static void setViewSize(View view, int widthPixels, int heightPixels) {
        ...
        if (params == null) {
            ISLogUtils.e(ISViewUtils.class, "setViewSize出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
