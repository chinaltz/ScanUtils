...
public class PermissionHelper {
    ...
    private static void request(final OnPermissionGrantedListener grantedListener,
                                final OnPermissionDeniedListener deniedListener,
                                final @PermissionConstants.Permission String... permissions) {
        ISPermissionUtils.permission(permissions)
                .rationale(new ISPermissionUtils.OnRationaleListener() {
                    @Override
                    public void rationale(ShouldRequest shouldRequest) {
                        ISPermissionDialogHelper.showRationaleDialog(shouldRequest);
                    }
                })
                .callback(new ISPermissionUtils.FullCallback() {
                    ...
                    public void onGranted(List<String> permissionsGranted) {
                        ...
                        ISLogUtils.d(permissionsGranted);
                    }

                    @Override
                    public void onDenied(List<String> permissionsDeniedForever, List<String> permissionsDenied) {
                        if (!permissionsDeniedForever.isEmpty()) {
                            ISPermissionDialogHelper.showOpenAppSettingDialog();
                        }
                        if (deniedListener != null) {
                            deniedListener.onPermissionDenied();
                        }
                        ISLogUtils.d(permissionsDeniedForever, permissionsDenied);
                    }
                })
                .request();
    }
