...
import com.developer.androidutilslib.model.ISAppProcessInfo;
import com.developer.androidutilslib.model.ISCPUInfo;
import com.developer.androidutilslib.model.ISProcessInfo;
import com.developer.androidutilslib.model.ISPsRow;
...
import static com.developer.androidutilslib.utils.Utils.getApp;
...
public class ISAppUtils {
     ...
     * @return
     */
    public static List<ISAppProcessInfo> getRunningAppProcesses(Context context) {
        ActivityManager activityManager = null;
        List<ISAppProcessInfo> list = null;
        PackageManager packageManager = null;
        try {
            activityManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
            packageManager = context.getApplicationContext().getPackageManager();
            list = new ArrayList<ISAppProcessInfo>();
            //所有运行的进程
            List<RunningAppProcessInfo> appProcessList = activityManager.getRunningAppProcesses();
            ApplicationInfo appInfo = null;
            ISAppProcessInfo abAppProcessInfo = null;
            ...
            for (RunningAppProcessInfo appProcessInfo : appProcessList) {
                abAppProcessInfo = new ISAppProcessInfo(appProcessInfo.processName, appProcessInfo.pid, appProcessInfo.uid);
                if (appInfo != null) {
                ...
                ISProcessInfo processInfo = getMemInfo(appProcessInfo.processName);
    ...
    public static void killProcesses(Context context, int pid, String processName) {
    	...
    	AbLogUtil.d(ISAppUtils.class, "#kill -9 "+pid);*/
     ...
     * 执行PS.
     */
    public static List<ISPsRow> ps() {
        List<ISPsRow> psRowlist = new ArrayList<ISPsRow>();
        String ps = runScript("ps");
        String[] lines = ps.split("\n");
        psRowlist = new ArrayList<ISPsRow>();
        for (String line : lines) {
            ISPsRow row = new ISPsRow(line);
     ...
     * @return
     */
    public static ISPsRow getPsRow(String processName) {
        List<ISPsRow> psRowlist = ps();
        for (ISPsRow row : psRowlist) {
     ...
     * @return
     */
    public static ISProcessInfo getMemInfo(String processName) {
        ISProcessInfo process = new ISProcessInfo();
        if (mProcessList == null) {
            mProcessList = getProcessRunningInfo();
        }
        ...
        for (Iterator<String[]> iterator = mProcessList.iterator(); iterator.hasNext(); ) {
            ...
            //AbLogUtil.d(ISAppUtils.class, "##"+item[9]+",NAME:"+processNameTemp);
            if (processNameTemp != null && processNameTemp.equals(processName)) {
                //AbLogUtil.d(ISAppUtils.class, "##"+item[9]+","+process.memory);
                //Process ID
        ...
        if (process.memory == 0) {
            ISLogUtils.i(ISAppUtils.class, "##" + processName + ",top -n 1未找到");
        }
        return process;
    }
     ...
     * @return
     */
    public static ISProcessInfo getMemInfo(int pid) {
        ISProcessInfo process = new ISProcessInfo();
        if (mProcessList == null) {
            mProcessList = getProcessRunningInfo();
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            //AbLogUtil.d(ISAppUtils.class, "##"+item[9]+",PID:"+tempPid);
    ...
    public static String runCommand(String[] command, String workdirectory) {
        String result = "";
        ISLogUtils.i(ISAppUtils.class, "#" + command);
        try {
     ...
     * @return
     */
    public static ISCPUInfo getCPUInfo() {
        ISCPUInfo CPUInfo = null;
        try {
            String result = runCommandTopN1();
    ...
    public static List<String[]> parseProcessRunningInfo(String info) {
        ...
        for (int i = 0; i < rows.length; i++) {
            tempString = rows[i];
            //AbLogUtil.d(ISAppUtils.class, tempString);
            if (tempString.indexOf("PID") == -1) {
                if (bIsProcInfo == true) {
                    ...
                    if (columns.length == Length_ProcStat) {
                        ...
                        //AbLogUtil.d(ISAppUtils.class, "#"+columns[9]+",PID:"+columns[0]);
     ...
     * @return
     */
    public static ISCPUInfo parseCPUInfo(String info) {
        ISCPUInfo CPUInfo = new ISCPUInfo();
        String tempString = "";
        String[] rows = null;
        String[] columns = null;
        rows = info.split("[\n]+");
    ...
    public static long getTotalMemory(Context context) {
        ...
        try {
            ...
            for (String str : strs) {
                ISLogUtils.i(ISAppUtils.class, str + "\t");
    ...
    public static boolean requestSDCardPermission(Context context) {
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale((Activity) context,
                ...
                ISToastUtils.showShort("SDCard访问权限被拒绝，请在设置-应用程序打开! ");
                return false;
            } else {
                //进行权限请求
    ...
    public static void clearAllCache(Context context) {
        try {
            PackageInfo info = ISAppUtils.getPackageInfo(context);
            File cacheDir = null;
            if (!ISFileUtils.isCanUseSD()) {
                cacheDir = new File(context.getCacheDir(), info.packageName);
            } else {
                cacheDir = new File(ISFileUtils.getCrashDir(context));
            }
    ...
    public static void clearMyRoot(Context context) {
        try {
            ...
            if (!ISFileUtils.isCanUseSD()) {
                ...
                downloadDir = new File(ISFileUtils.getMyRootDir(context));
            }
...
//			AbLogUtil.d(AbDiskCacheImpl.class,"清除缓存失败.");
    ...
    public static void installApp(final File file) {
        ...
        getApp().startActivity(ISIntentUtils.getInstallAppIntent(file, true));
    }
    ...
    public static void installApp(final File file, final String authority) {
        if (!isFileExists(file)) return;
        getApp().startActivity(ISIntentUtils.getInstallAppIntent(file, authority, true));
    }
    ...
    public static void installApp(final Activity activity,
                                  ...
                                  final int requestCode) {
        if (!isFileExists(file)) return;
        activity.startActivityForResult(ISIntentUtils.getInstallAppIntent(file), requestCode);
    }
    ...
    public static void installApp(final Activity activity,
                                  ...
                                  final int requestCode) {
        if (!isFileExists(file)) return;
        activity.startActivityForResult(ISIntentUtils.getInstallAppIntent(file, authority),
                requestCode);
    }
    ...
    public static boolean installAppSilent(final File file,
        ...
        String command = "LD_LIBRARY_PATH=/vendor/lib*:/system/lib* pm install " +
                (params == null ? "" : params + " ")
                + filePath;
        ISShellUtils.CommandResult commandResult = ISShellUtils.execCmd(command, isRooted);
    ...
    public static void uninstallApp(final String packageName) {
        ...
        getApp().startActivity(ISIntentUtils.getUninstallAppIntent(packageName, true));
    }
    ...
    public static void uninstallApp(final Activity activity,
        ...
        activity.startActivityForResult(
                ISIntentUtils.getUninstallAppIntent(packageName),
                requestCode
        );
    }
    ...
    public static boolean isAppInstalled(@NonNull final String packageName) {
        return !isSpace(packageName) && ISIntentUtils.getLaunchAppIntent(packageName) != null;
    }
    ...
    public static boolean isAppRoot() {
        ISShellUtils.CommandResult result = ISShellUtils.execCmd("echo root", true);
    ...
    public static boolean isAppForeground(@NonNull final String packageName) {
        return !isSpace(packageName) && packageName.equals(ISProcessUtils.getForegroundProcessName());
    }
    ...
    public static void launchApp(final String packageName) {
        if (isSpace(packageName)) return;
        getApp().startActivity(ISIntentUtils.getLaunchAppIntent(packageName, true));
    }
    ...
    public static void launchApp(final Activity activity,
                                 ...
                                 final int requestCode) {
        if (isSpace(packageName)) return;
        activity.startActivityForResult(ISIntentUtils.getLaunchAppIntent(packageName), requestCode);
    }
    ...
    public static void launchAppDetailsSettings(final String packageName) {
        ...
        getApp().startActivity(
                ISIntentUtils.getLaunchAppDetailsSettingsIntent(packageName, true)
        );
    }
