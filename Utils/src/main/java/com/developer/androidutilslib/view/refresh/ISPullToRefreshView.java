 ...
 * Info 下拉刷新和加载更多的View
 */
public class ISPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private ISListViewBaseHeader mHeaderView;
    private ISPullHeader defaultViewHeader;

    /** footer view. */
	private ISPullFooter mFooterView;
	 ...
	 */
	public ISPullToRefreshView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	 ...
	 * @param context the context
	 */
	public ISPullToRefreshView(Context context) {
		super(context);
		init(context);
	}
	...
	private void init(Context context) {
        ...
        defaultViewHeader = new ISPullHeader(mContext);
		setHeaderView(defaultViewHeader);
	}

    public ISPullHeader getDefaultHeaderView() {
	 ...
	 * 增加头布局.
	 */
	public void setHeaderView(ISListViewBaseHeader headerView) {
...
//		ISViewUtil.measureView(this.mHeaderView);
	...
	private void addFooterView() {
        ...
        mFooterView = new ISPullFooter(mContext);
	...
	private void initContentAdapterView() {
		...
		if (mAdapterView == null && mScrollView == null&& mRecyclerView == null&&nestedScrollView==null) {
			throw new IllegalArgumentException("must contain a AdapterView or ScrollView or RecyclerView  or NestedScrollViewin this layout!");
		}
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			//解决点击与移动的冲突
			if(Math.abs(deltaX)< Math.abs(deltaY) && Math.abs(deltaY) > 6){
...
//			}
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != ISListViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(ISListViewHeader.STATE_READY);

		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(ISListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != ISListViewFooter.STATE_LOADING) {
			mFooterView.setState(ISListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(ISListViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(ISListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(ISListViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(ISListViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(ISListViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(ISListViewFooter.STATE_READY);
		}
     ...
     */
    public ISListViewBaseHeader getHeaderView() {
        return mHeaderView;
    }
     ...
     */
    public ISPullFooter getFooterView() {
        return mFooterView;
    }
	...
	public interface OnFooterLoadListener {
		 ...
		 * @param view the view
		 */
		public void onFooterLoad(ISPullToRefreshView view);
	}
	...
	public interface OnHeaderRefreshListener {
		 ...
		 * @param view the view
		 */
		public void onHeaderRefresh(ISPullToRefreshView view);
	}
